...
[role="_abstract"]
An {product-title} cluster can be made private so that applications cannot be accessed from the Internet. In addition, private clusters can be configured to have only internal API endpoints for increased security.

//{product-title} administrators can choose between public and private cluster configuration from within the OpenShift Cluster Manager (OCM) or the `rosa` CLI.
